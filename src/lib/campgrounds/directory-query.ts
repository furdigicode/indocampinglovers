...
export type DirectoryRegencyOption = DirectoryOption & {
  province: string;
};

export type CampgroundDirectoryFacets = {
  provinces: DirectoryOption[];
  regencies: DirectoryRegencyOption[];
  types: DirectoryOption[];
  facilities: DirectoryOption[];
  access: DirectoryOption[];
};

export type CampgroundDirectoryResult<T> = {
  items: T[];
  total: number;
  page: number;
  pageSize: number;
