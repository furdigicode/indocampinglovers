...
export async function getProvinceLanding(slug: string): Promise<ProvinceLanding | null> {
  ...
  const regencyRows = (regencies ?? []) as RegencyGeography[];
  const regencyCounts = await Promise.all(
    regencyRows.map(async (regency) => {
  ...
  return {
    ...province,
    publishedCampgroundCount: normalizeCount(provinceCount),
    regencies: regencyCounts,
  };
}
...
export async function getRegencyLanding(provinceSlug: string, regencySlug: string): Promise<RegencyLanding | null> {
  ...
  const { data, error } = await supabase
    .from("regencies")
    .select("id,code,name,slug,type,province_id")
  ...
  const regency = data as RegencyGeography;
  const { count, error: countError } = await supabase
    .from("campgrounds")
    .select("id", { count: "exact", head: true })
    .eq("regency_id", regency.id);
  ...
  return {
    ...regency,
    province,
    publishedCampgroundCount: normalizeCount(count),
  };
}
