  ...
  return (
    <form action="/camping" method="get" className="mt-6 rounded-3xl border border-black/10 bg-white p-4 shadow-sm md:p-5">
      ...
      <div className="hidden md:block">
        <div className="flex items-center gap-2 font-bold text-forest-900"><SlidersHorizontal size={18}/>Filter {activeCount > 0 && <span className="rounded-full bg-forest-800 px-2 py-0.5 text-xs text-white">{activeCount}</span>}</div>
        {fields}
      </div>
      <details className="group md:hidden" open={activeCount > 0 ? true : undefined}>
        <summary className="flex cursor-pointer list-none items-center justify-between gap-4 font-bold text-forest-900">
          ...
          <span className="text-xs font-semibold text-forest-700">Buka / tutup</span>
        </summary>
        {fields}
      </details>
    </form>
