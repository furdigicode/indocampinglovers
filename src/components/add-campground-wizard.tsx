...
export function AddCampgroundWizard({ reference }: { reference: ContributionReferenceData }) {
  ...
  return <form onSubmit={submit} className="rounded-3xl border border-black/5 bg-white p-6 shadow-sm md:p-10">
    ...
    <div className="mt-9">
      {step===0&&<div className="space-y-5"><div><p className="icl-eyebrow">Langkah 1</p><h1 className="mt-2 text-3xl font-extrabold">Tempat campingnya di mana?</h1><p className="mt-2 text-sm text-black/55">Mulai dari informasi dasar. Semua field di langkah ini wajib.</p></div>
        <Field label="Nama tempat camping"><input value={form.name} onChange={e=>set("name",e.target.value)} className="input" placeholder="Contoh: Camping Ground ..." /></Field>
        <div className="grid gap-4 sm:grid-cols-2"><Field label="Provinsi"><select value={form.provinceId} onChange={e=>{setForm(p=>({...p,provinceId:e.target.value,regencyId:""}))}} className="input"><option value="">Pilih provinsi</option>{reference.provinces.map(x=><option key={x.id} value={x.id}>{x.name}</option>)}</select></Field><Field label="Kabupaten / Kota"><select value={form.regencyId} disabled={!form.provinceId} onChange={e=>set("regencyId",e.target.value)} className="input"><option value="">Pilih kabupaten/kota</option>{regencies.map(x=><option key={x.id} value={x.id}>{x.name}</option>)}</select></Field></div>
        <Field label="Alamat / petunjuk lokasi"><textarea value={form.address} onChange={e=>set("address",e.target.value)} className="input min-h-24" placeholder="Desa, kecamatan, jalan atau petunjuk menuju lokasi" /></Field></div>}
      {step===1&&<div className="space-y-6"><div><p className="icl-eyebrow">Langkah 2</p><h2 className="mt-2 text-3xl font-extrabold">Apa yang kamu tahu?</h2><p className="mt-2 text-sm text-black/55">Bagian ini boleh dilewati. Isi informasi yang kamu yakin benar.</p></div>
        ...
        <div className="grid gap-4 sm:grid-cols-2"><Field label="Harga mulai (Rp) — opsional"><input type="number" min="0" value={form.price} onChange={e=>set("price",e.target.value)} className="input" placeholder="25000"/></Field><Field label="Satuan harga"><input value={form.priceUnit} onChange={e=>set("priceUnit",e.target.value)} className="input"/></Field></div>
        <Field label="Catatan tambahan — opsional"><textarea value={form.notes} onChange={e=>set("notes",e.target.value)} className="input min-h-24" placeholder="Jam buka, kondisi jalan, aturan lokasi, kontak, atau informasi lain"/></Field></div>}
      ...
      {step===3&&<div className="space-y-5"><div><p className="icl-eyebrow">Langkah 4</p><h2 className="mt-2 text-3xl font-extrabold">Terakhir, tentang kamu</h2><p className="mt-2 text-sm text-black/55">Kami memakai kontak ini hanya bila perlu mengklarifikasi kontribusi.</p></div>
        <Field label="Nama"><input value={form.submitterName} onChange={e=>set("submitterName",e.target.value)} className="input" /></Field><Field label="Email / WhatsApp"><input value={form.submitterContact} onChange={e=>set("submitterContact",e.target.value)} className="input" /></Field>
